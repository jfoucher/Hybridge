/*
 * Sector — mechanical-style watchface: heart-rate and calorie sub-eyes,
 * a steps sub-dial with a goal-progress ring, and a date window under
 * 12 o'clock.  Time is told by the physical hands only.
 *
 * Packaged as a watchface (version 1.x.y.z).
 */
return {
	"node_name": '',
	...
	"compute": function () {
		...
		var bpm = c.hr_bpm;
		return {
			"json_file": 'sector_layout',
			"sarc": Math.max(2, Math.round(frac * 360)),
