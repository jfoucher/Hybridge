/*
 * Gazette — the day as a broadsheet front page: dateline, steps as
 * the headline, calories on the second deck, and weather / battery
 * in the inset boxes.  Weather comes from the companion app's
 * weatherInfo answer and reads "--" until it arrives.
 */
return {
	"node_name": '',
	...
	"compute": function () {
		...
		var dow = this.day_of_week(c.year || 2026, month, c.date || 1);
		return {
			"json_file": 'gazette_layout',
			"dateline": this.dows[dow].slice(0, 3) + ', '
				+ this.months[month - 1].slice(0, 3) + ' ' + (c.date || 1),
			"head1": this.sep(c.step_count) + ' STEPS',
			"head2": (c.calories || 0) + ' KCAL',
			"wx": w
