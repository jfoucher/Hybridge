/*
 * Reserve — dress complication dial: Roman numerals, a moonphase
 * sub-dial under 12 (computed from the date, no phone needed), a
 * power-reserve fan along the bottom rim showing battery, and a date
 * window above 6.
 */
return {
	"node_name": '',
	...
	"compute": function () {
		...
		return {
			"json_file": 'reserve_layout',
			"moon": 'phase' + this.moon_phase(c.year || 2026,
				(c.month || 0) + 1, c.date || 1),
			"barc": 120 + Math.max(2, Math.round(soc * 1.2)),
			"date": '' + (c.date || 1)
		};
	},
