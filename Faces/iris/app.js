/*
 * Iris — a giant eye.  The hands hub hides inside the pupil, and the
 * catch-light glint orbits the iris to point where the minute hand
 * points.  Date at the bottom lid.
 */
return {
	"node_name": '',
	...
	"compute": function () {
		...
		var a = (this.local_minutes(c) % 60) * 6 * Math.PI / 180;
		return {
			"json_file": 'iris_layout',
			"gx": Math.round(120 + 34 * Math.sin(a)) - 6,
			"gy": Math.round(120 - 34 * Math.cos(a)) - 6,
			"date": '' + (c.date || 1)
		};
	},
