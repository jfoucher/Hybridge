/*
 * TTY — a tiny terminal: scanlines, a mono header, and a prompt per
 * data line (date, steps, calories, heart rate, battery).
 */
return {
	"node_name": '',
	...
	"compute": function () {
		...
		var dow = this.day_of_week(c.year || 2026, (c.month || 0) + 1,
			c.date || 1);
		return {
			"json_file": 'tty_layout',
			"l1": this.dows[dow] + ' ' + (c.date || 1) + ' '
				+ this.mons[c.month || 0],
			"l2": 'STEP ' + (c.step_count || 0),
			"l3": 'KCAL ' + (c.calories || 0),
			"l4": 'BPM ' + ((bpm && bpm > 0) ? Math.round(bpm) : '--'),
			"l5": 'BATT ' + (c.battery_soc || 0) + '%'
		};
	},
