/*
 * Retro — twin retrograde gauges, the watchmaker's party trick: a
 * steps needle and a battery needle each sweep a 120-degree fan and
 * fly back as the value resets.  Date window at 6.
 */
return {
	"node_name": '',
	...
	"compute": function () {
		...
		var frac = (c.step_count || 0) / goal;
		if (frac > 1) frac = 1;
		var soc = (c.battery_soc || 0) / 100;
		return {
			"json_file": 'retro_layout',
			"srot": (Math.round(-60 + frac * 120) + 360) % 360,
			"brot": (Math.round(-60 + soc * 120) + 360) % 360,
			"date": '' + (c.date || 1)
		};
	},
