/*
 * Gnomon — a sundial.  The shadow bar sweeps the stone dial once per
 * day (noon at the top, midnight below), and the date is engraved in
 * Roman numerals.  The physical hands still tell civil time.
 */
return {
	"node_name": '',
	...
	// 1..39 is plenty for a day of month
	"roman": function (n) {
		var vals = [10, 9, 5, 4, 1];
	...
	"compute": function () {
		...
		return {
			"json_file": 'gnomon_layout',
			"grot": Math.round(mins / 1440 * 360 + 180) % 360,
			"date": this.roman(c.date || 1)
		};
	},
