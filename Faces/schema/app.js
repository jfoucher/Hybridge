/*
 * Schema — an engineering drawing of the watch itself: frame, center
 * mark on the hands hub, a leader note, and the data (date, steps,
 * battery) in the title block, as any proper drawing would have it.
 */
return {
	"node_name": '',
	...
	"compute": function () {
		...
		return {
			"json_file": 'schema_layout',
			"date": '' + (c.date || 1),
			"steps": this.fmt_steps(c.step_count),
			"batt": (c.battery_soc || 0) + '%'
		};
	},
