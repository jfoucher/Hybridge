/*
 * Radar — range rings and a sweep line that tracks the minute (SVG
 * path rotated at runtime), with steps and heart rate as HUD text.
 */
return {
	"node_name": '',
	...
	"compute": function () {
		...
		return {
			"json_file": 'radar_layout',
			"sweep": minute * 6,
			"hud1": this.fmt_steps(c.step_count) + ' STEPS',
			"hud2": ((bpm && bpm > 0) ? '' + Math.round(bpm) : '--')
				+ ' BPM'
		};
	},
