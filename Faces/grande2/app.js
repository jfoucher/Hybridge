/*
 * Grande II — the grand complication, sized for the wrist: the same
 * thirteen analog complications on large dials (calendar with
 * moonphase aperture and coaxial date+weekday hands, steps+active
 * coaxial pair, heart rate, temperature, UV), a month fan and three
 * rim gauges (calories, rain chance, power reserve).  Temperature,
 * rain and UV come from the phone via the req_data("weatherInfo")
 * round-trip; their hands rest at the low end until data arrives.
 */
return {
	"node_name": '',
	...
	// the phone's answer, or null while it is stale/absent
	"weather": function (c) {
		var w = c.weatherInfo;
	...
	"compute": function () {
		...
		// temperature dial reads -10..40 C; the phone may send F
		var temp = w
			? (w.unit === 'f' ? (w.temp - 32) * 5 / 9 : w.temp)
			: -10;
		var uv = (w && typeof w.uv === 'number') ? w.uv : 0;
		var rain = (w && typeof w.rain === 'number') ? w.rain : 0;
		return {
			"json_file": 'grande2_layout',
			"moon": 'phase' + this.moon_phase(c.year || 2026, month, date),
			"daterot": Math.round((date - 1) * 360 / 31),
			"dowrot": Math.round(dow * 360 / 7),
			"monrot": (Math.round(-60 + (month - 1) * 120 / 11) + 360) % 360,
			...
			"temprot": this.gauge((temp + 10) / 50),
			"uvrot": this.gauge(uv / 11),
			"rarc": 90 + Math.max(1, Math.round(
				Math.min(rain / 100, 1) * 60)),
