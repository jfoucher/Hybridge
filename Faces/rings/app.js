/*
 * Rings — three concentric goal rings: steps vs daily goal (outer,
 * white), battery (middle, grey), active minutes vs 60 (inner, white),
 * with the numeric values stacked in the middle of the dial.
 */
return {
	"node_name": '',
	...
	// the tracks are broken at 12 o'clock (14deg gap each side) so the
	// glyphs stay visible; the value arcs run 14..346 to match
	"ring": function (frac) {
		if (frac > 1) frac = 1;
		if (frac < 0) frac = 0;
		return Math.max(16, Math.round(14 + frac * 332));
	},
	...
	"compute": function () {
		...
		return {
			"json_file": 'rings_layout',
			"sarc": this.ring(steps / goal),
			"barc": this.ring(soc / 100),
			"aarc": this.ring(act / 60),
			"steps": this.fmt_steps(steps),
			"batt": soc + '%',
			"act": act + 'm'
		};
	},
