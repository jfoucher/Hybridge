/*
 * Glass — an hourglass for the day itself: the top bulb holds what is
 * left of today, the bottom bulb what has passed.  Sand is two
 * masked solids behind transparent bulb windows.
 */
return {
	"node_name": '',
	...
	"compute": function () {
		...
		var f = this.local_minutes(c) / 1440;
		var th = Math.round((1 - f) * 60);
		var bh = Math.round(f * 68);
		var dow = this.day_of_week(c.year || 2026, (c.month || 0) + 1,
			c.date || 1);
		return {
			"json_file": 'glass_layout',
			"t1t": 106 - th,
			"t1h": Math.max(1, th),
			"t2t": 202 - bh,
			"t2h": Math.max(1, bh),
