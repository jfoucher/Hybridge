/*
 * Aria - two delicate hairline arcs trace your steps and battery around a refined dial, with the readings set fine.
 */
return {
	"node_name": '',
	...
	"compute": function () {
		...
		return {
			"json_file": 'aria_layout',
			"sarc": Math.max(2, Math.round(clamp((c.step_count || 0) / goal) * 340)),
			"barc": Math.max(2, Math.round(soc / 100 * 340)),
			"sval": this.fmt_steps(c.step_count),
			"bval": soc + '%',
